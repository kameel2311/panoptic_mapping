...
def create_predictions(params: Params, device="cpu"):
    ...
    files = [
        o
        for o in os.listdir(params.target_path)
    ...
    for i, im_file in enumerate(files):
        ...
        with open(
            os.path.join(params.target_path, file_id + "_labels.json"), "w"
        ) as json_file:
    ...
    print(
        "Average inference time was %.1f +/- %.1f ms per frame."
...
if __name__ == "__main__":
    ...
    params.target_path = "/workspace/Datasets/docking_3_sync_aligned_resized/run1"
